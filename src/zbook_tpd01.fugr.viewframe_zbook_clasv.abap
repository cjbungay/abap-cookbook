*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZBOOK_CLASV
*   generation date: 20.09.2012 at 16:54:51 by user EWULFF
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZBOOK_CLASV         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZBOOK_CLASV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZBOOK_CLASV_EXTRACT
TOTAL = ZBOOK_CLASV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZBOOK_CLASV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZBOOK_CLASV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZBOOK_CLASV_EXTRACT
TOTAL = ZBOOK_CLASV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZBOOK_CLASV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZBOOK_CLASV'
...
TOTAL = ZBOOK_CLASV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZBOOK_CLASV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZBOOK_CLASV-UPD_FLAG NE SPACE.
